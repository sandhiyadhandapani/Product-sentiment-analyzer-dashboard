...
def test_product_consistency():
    ...
    if len(successful_runs) > 1:
        ...
        total_ratings = set(r['total_ratings'] for r in successful_runs if r['total_ratings'])
        ...
        print(f"Unique total_ratings: {len(total_ratings)}")
        ...
        if len(total_ratings) == 1:
            print("✓ Total ratings is CONSISTENT across runs")
        else:
            print("✗ Total ratings is INCONSISTENT across runs")
            print(f"  Found: {total_ratings}")
    ...
    print("=" * 80)
    for r in results:
        print(f"Run {r['run']}: name={r['product_name']}, price={r['current_price']}, rating={r['rating']}, total_ratings={r['total_ratings']}, reviews={r['reviews_count']}, success={r['success']}, time={r['elapsed']:.2f}s")
    
    # Save results to file
    with open('test_consistency_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print("\nResults saved to test_consistency_results.json")

if __name__ == "__main__":
    test_product_consistency()
